...
class PrimitiveShapeField(ABC):
    ...
    def set_velocities(self, velocities):
        """Set velocities for all primitives in the field"""
        self.velocities = velocities  # Shape: (n_primitives, dim)
        
    def set_ids(self, ids):
        """Set unique IDs for all primitives"""
        self.ids = ids
        ...
        for i in range(len(self.centers)):
            for dim in range(len(self.centers[i])):
                if self.centers[i][dim] < limits[0][dim]:
                    ...
                    self.centers[i][dim] = limits[1][dim]
                    self.velocities[i][dim] *= -1  # Bounce off boundary
...
class MultiSphereField(PrimitiveShapeField):
    ...
    def __init__(self, centers, radii, tensor_args=None, velocities=None, ids=None):
        """
        Parameters
        ----------
            centers : numpy array
        ...
        if velocities is not None:
            self.set_velocities(to_torch(velocities, **self.tensor_args))
        if ids is not None:
            self.set_ids(ids)
    ...
    def render(self, ax, pos=None, ori=None, color='gray', cmap='gray', **kwargs):
        ...
        if self.velocities is not None:
            ...
            for center, velocity in zip(self.centers, self.velocities):
                center = to_numpy(center)
